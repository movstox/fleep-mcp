...
- Create new Fleep conversations with specified members and topics
- Send messages to existing Fleep conversations
- Retrieve labels applied to a Fleep conversation
- Apply labels to a Fleep conversation
...
If you're using `uv` for dependency management, you can configure Claude to use it. For local setups, it's recommended to use bash to ensure proper directory navigation:
...
```json
{
  "mcpServers": {
    "fleep-mcp": {
      ...
      "args": [
        "-c",
        "cd /path/to/your/fleep-mcp && uv run python -m src.main"
      ],
      "env": {
        "FLEEP_EMAIL": "your-email@example.com",
        "FLEEP_PASSWORD": "your-password"
...
**Alternative simpler configuration (may not work in all environments):**
```json
{
  "mcpServers": {
    "fleep-mcp": {
...
- Automatic dependency management and virtual environment handling
...
- Run `uv sync` in the project directory before using this configuration
- Replace `/path/to/your/fleep-mcp` with the actual path to your fleep-mcp directory
- For local setups, the bash command approach ensures proper directory context
